...
- Keep a sane heading hierarchy and landmark structure — one `h1` per view, no skipped levels, content inside landmarks.
...
- After an action removes or replaces the focused element (delete a row, close a panel), move focus somewhere sensible — never let it drop to `<body>`.
...
- Honor `prefers-reduced-motion` for all non-trivial animation (mechanics per styling approach — see `styling.md`).
