...
1. **Pin behavior.** Run `<pm> run test` first — it must be green. Red already? Stop and surface it — don't refactor on a red suite. If the area is uncovered, add **characterization tests** that capture current behavior before touching anything (the safety net for "no behavior change").
