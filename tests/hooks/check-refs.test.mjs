...
import { cleanup, fixture, runHook } from './helpers.mjs'
...
test('usage error: no names → exit 2', () => {
  assert.equal(runHook('check-refs.mjs', { root: corpus() }).status, 2)
})

test('finds a live reference with file:line and exits 1', () => {
  const res = runHook('check-refs.mjs', { root: corpus(), args: ['devil'] })
...
test('CHANGELOG and the prune catalog are excluded from the corpus', () => {
  ...
  assert.equal(runHook('check-refs.mjs', { root, args: ['devil'] }).status, 0)
})

test('clean name exits 0; hyphen-glued words do not count as references', () => {
  const root = corpus()
  assert.equal(runHook('check-refs.mjs', { root, args: ['ghost-unit'] }).status, 0)
  // docs/notes.md mentions release-automation, which must NOT match "release"
  assert.equal(runHook('check-refs.mjs', { root, args: ['release'] }).status, 0)
})
